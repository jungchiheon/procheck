  ...
  useEffect(() => {
    ;(async () => {
      ...
      try {
        ...
        // partner list (API: admin이면 staff 목록)
      ...
      {!loading && !error && (
        <GlassCard className="p-2">
          <div className="divide-y divide-white/10">
            {filtered.length === 0 && <div className="py-10 text-sm text-white/60 text-center">직원이 없습니다.</div>}
            ...
            {filtered.map((u) => {
              ...
              return (
                <button
                  key={u.id}
                  type="button"
                  onClick={() => router.push(`/admin/chat/${u.id}`)}
                  className={cn('w-full text-left px-4 py-4 hover:bg-white/5 transition')}
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="text-white font-semibold truncate">{u.nickname ?? u.login_id ?? '직원'}</div>
